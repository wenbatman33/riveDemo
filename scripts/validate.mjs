...
for (const d of catalog) {
  ...
  assert(d.title && (d.src || d.kind === 'grid'), `Missing fields: ${d.id}`);
  for (const file of [d.src, d.editable, d.controller, ...(d.items || []).map(i => i.src), ...(d.choices || []).flatMap(c => [c.src, c.editable])].filter(Boolean)) {
  ...
  if (d.kind === 'grid') {
    assert(d.items.length && new Set(d.items.map(i => i.id)).size === d.items.length);
    assert(d.items.every(i => i.src), 'Each grid item needs its own Rive file');
    assert(new Set(d.items.map(i => i.src)).size === d.items.length, 'Grid files must be distinct');
  }
